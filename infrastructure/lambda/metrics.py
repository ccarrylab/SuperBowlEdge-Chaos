...
# Allow overriding IDs via env vars (defaults keep your current behavior)
CF_DIST_ID = os.environ.get('CF_DIST_ID', 'E3EHW9FZ4D82AY')
ALB_ARN_SUFFIX = os.environ.get('ALB_ARN_SUFFIX', 'app/superbowl-edge-dev-alb/4de49584541c5284')

# SportsDataIO creds via env vars (do NOT hardcode keys in code)
SPORTSDATA_KEY = os.environ.get('SPORTSDATA_KEY', '')

# SportsDataIO URL (override if you want)
# Example endpoint (you may need to change the season string): /v3/nfl/scores/json/Scores/2026POST
SPORTSDATA_BASE_URL = os.environ.get('SPORTSDATA_BASE_URL', 'https://api.sportsdata.io')
SPORTSDATA_SCORES_PATH = os.environ.get('SPORTSDATA_SCORES_PATH', '/v3/nfl/scores/json/Scores/2026POST')
...
def lambda_handler(event, context):
    ...
    # HTTP API v2 uses rawPath; add safe fallback for direct invokes/REST API
    ...
    # NFL scores route (matches /nfl-scores and /prod/nfl-scores)
    if 'nfl-scores' in path:
...
# =============================================================================
# Chaos Experiment Control Functions
# =============================================================================
    ...
    try:
        ...
        if not template_id:
            return error_response(
                400,
                f'Unknown experiment type: {experiment_type}. Available: {list(EXPERIMENT_TEMPLATES.keys())}'
            )
    ...
    try:
        ...
        for action_name, action_data in exp.get('actions', {}).items():
            ...
            actions.append({
                ...
                'startTime': start_time.isoformat() if hasattr(start_time, 'isoformat') else (str(start_time) if start_time else None),
                'endTime': end_time.isoformat() if hasattr(end_time, 'isoformat') else (str(end_time) if end_time else None)
            })
...
def get_experiment_templates():
    ...
    try:
        ...
        return success_response({'templates': templates, 'count': len(templates)})
...
def get_cloudfront_metrics():
    ...
    try:
        ...
        requests = get_metric('AWS/CloudFront', 'Requests', CF_DIST_ID, start_time, end_time)
        bandwidth = get_metric('AWS/CloudFront', 'BytesDownloaded', CF_DIST_ID, start_time, end_time)
        error_4xx = get_metric('AWS/CloudFront', '4xxErrorRate', CF_DIST_ID, start_time, end_time, stat='Average')
        error_5xx = get_metric('AWS/CloudFront', '5xxErrorRate', CF_DIST_ID, start_time, end_time, stat='Average')
...
def get_cloudfront_regional_metrics():
    ...
    try:
        ...
        total_requests = get_metric('AWS/CloudFront', 'Requests', CF_DIST_ID, start_time, end_time)
        total_bandwidth = get_metric('AWS/CloudFront', 'BytesDownloaded', CF_DIST_ID, start_time, end_time)
        ...
        for region, config in regions.items():
            ...
            reqs = int((total_requests / 300) * config['weight'] * (1 + variance))
            bw = int((total_bandwidth / 1024 / 1024 / 300) * config['weight'] * (1 + variance))
            latency = int(config['latency_base'] * (1 + variance * 0.2))
            hit_rate = round(config['hitRate_base'] * (1 + variance * 0.05), 1)
            ...
            regional_data.append({
                ...
                'hitRate': max(85, min(99, hit_rate)),
                'timestamp': datetime.utcnow().isoformat()
            })
...
def get_infrastructure_status():
    ...
    try:
        ...
        running = sum(1 for i in instances if i.get('LifecycleState') == 'InService')
        healthy = sum(1 for i in instances if i.get('HealthStatus') == 'Healthy')
        ...
        return success_response({
            'haproxy': {
                ...
                'desiredCapacity': asg_data.get('DesiredCapacity'),
                'minSize': asg_data.get('MinSize'),
...
# =============================================================================
# NFL Scores (SportsDataIO)
# =============================================================================

def get_nfl_scores():
    """NFL scores via SportsDataIO (Scores & Game State feed)."""
    try:
        if not SPORTSDATA_KEY:
            return success_response({
                'nfl': {'status': 'SPORTSDATA_KEY missing'},
                'timestamp': datetime.utcnow().isoformat()
            })

        url = SPORTSDATA_BASE_URL.rstrip('/') + SPORTSDATA_SCORES_PATH
        ...
        if isinstance(data, list) and data:
            game = next((g for g in data if g.get('Status') == 'InProgress'), data[0])

        return success_response({
            'nfl': {
                'away': f"{game.get('AwayTeam', 'N/A')}: {game.get('AwayScore', 0)}",
                'home': f"{game.get('HomeTeam', 'N/A')}: {game.get('HomeScore', 0)}",
                'status': game.get('Status', 'No games')
            },
            'timestamp': datetime.utcnow().isoformat()
        })
        ...
        return success_response({
            'nfl': {'status': 'SportsDataIO HTTPError', 'code': e.code},
            'timestamp': datetime.utcnow().isoformat()
        })
    except Exception as e:
        return success_response({
            'nfl': {'status': 'SportsDataIO fetch failed', 'error': str(e)[:200]},
            'timestamp': datetime.utcnow().isoformat()
        })
