...
# NEW (added): allow overriding IDs via env vars (defaults keep your current behavior)
...
# NEW (added): SportsDataIO creds via env vars (do NOT hardcode keys in code)
SPORTSDATA_KEY = os.environ.get('SPORTSDATA_KEY', '')
#MSF_PASSWORD = os.environ.get('MSF_PASSWORD', '')  # some accounts may use "MYSPORTSFEEDS"; leave blank if yours is blank
...
def lambda_handler(event, context):
    ...
    # Existing behavior: HTTP API v2 uses rawPath; add safe fallback for direct invokes/REST API
    ...
    # NEW (added): CORS preflight
    if method == 'OPTIONS':
        return success_response({'ok': True})

    # NEW (added): NFL scores route
    if path.endswith('/nfl-scores'):
...
def get_nfl_scores():
    """NFL scores: SportsDataIO (key 669e5a8a43b3423692bd42be989af571)"""
    try:
        import urllib.request, json
        url = "https://nfl.sportsdata.io/v3/nfl/scores/json/Scores/2026POST?key=669e5a8a43b3423692bd42be989af571"
        ...
        return success_response({'nfl': {'status': str(e)[:100]}})
